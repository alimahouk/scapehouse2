...
/**
 * Scapes access token table management class.
 *
 * @copyright  2014 Scapehouse
 */

class Scapes_Model_DbTable_Shaccesstoken extends Zend_Db_Table_Abstract
{
    protected $_name = "sh_scapes_access_token"; // The table name.
    ...
    /**
     * Updates the user presence associated with a session.
     *
     * @param int $tokenID The ID of the session.
     * @param int $presence The new presence value.
     *
     * @return void
     */
    function updateSessionPresence($tokenID, $presence)
    {
        $tokenIDQ = $this->_db->quoteInto("?", $tokenID);
        
        $this->_db->update($this->_name, array("session_presence" => $presence), "token_id = {$tokenIDQ}");
    }

    /**
     * Sets all sessions as offline. Used when the discovery server restarts.
     *
     * @return void
     */
    function resetSessionPresenceAll()
    {
        $this->_db->update($this->_name, array("session_presence" => 1));
    }
