...
class Scapes_Model_DbTable_Shspotted extends Zend_Db_Table_Abstract
{
    protected $_name = "sh_scapes_spotted"; // The table name.
    ...
    function getSpottedListWithUserData($userID)
    {
        $query = $this->_db->select()->from("sh_user")
                ->join("sh_scapes_spotted", "((sh_scapes_spotted.user_1 = sh_user.user_id AND sh_scapes_spotted.user_1 <> {$userID}) OR (sh_scapes_spotted.user_2 = sh_user.user_id AND sh_scapes_spotted.user_2 <> {$userID})) AND (sh_scapes_spotted.user_1 = {$userID} OR sh_scapes_spotted.user_2 = {$userID})", array())
