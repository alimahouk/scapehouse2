...
/**
 * Nightboard contact followership table management class.
 *
 * @copyright  2015 Scapehouse
 */

class Theboard_Model_DbTable_Shfollow extends Zend_Db_Table_Abstract
{
    protected $_name = "sh_follow"; // The table name.
    ...
    /**
     * Unsubscribes a user from a contact's updates.
     *
     * @param int $followerID The ID of the follower.
     * @param int $followedID The ID of the person followed.
     *
    ...
    function getFollowingListWithUserData($userID, $currentUserID = -1)
    {
        $query = $this->_db->select()->from("sh_user")
                ->join("sh_follow", "sh_follow.followed_userid = sh_user.user_id AND sh_follow.follower_userid = {$userID} AND sh_follow.followed_userid <> {$userID}", array())
                ->limit(500, 0);
    ...
    {
        $query = $this->_db->select()->from("sh_user")
                ->join("sh_follow", "sh_follow.followed_userid = {$userID} AND sh_follow.follower_userid = sh_user.user_id AND sh_follow.follower_userid <> {$userID}", array())
                ->limit(500, 0);
