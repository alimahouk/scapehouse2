...
//    if (strlen($dest) > 20) {
//
//        $passUrl = rawurlencode($dest);
//
//        $curl_handle = curl_init();
//        curl_setopt($curl_handle, CURLOPT_URL, "http://api.bit.ly/shorten?version=2.0.1&longUrl={$passUrl}&login=scapehouse&apiKey=R_e0dad6ddba68c1557cecc94eac34e512");
//        curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
//        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
//        $buffer = curl_exec($curl_handle);
//        curl_close($curl_handle); // Max size for short links = 30
//        // BitLying END
//
//        $bitlyResponce = (array) json_decode($buffer);
//        $results = (array) $bitlyResponce["results"];
//        $link = (array) $results[$dest];
//
//        return $matches[1] . $link["shortUrl"];
//    } else {
...
function _make_url_clickable_cb($matches) {
    ...
    if (strlen($url) > 20 && !$noBitly) { //echo $url . "<br>";
        $passUrl = rawurlencode($url);

        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, "http://api.bit.ly/shorten?version=2.0.1&longUrl={$passUrl}&login=scapehouse&apiKey=YOUR_BITLY_API_KEY");
        curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
        $buffer = curl_exec($curl_handle);
        curl_close($curl_handle); // Max size for short links = 30
